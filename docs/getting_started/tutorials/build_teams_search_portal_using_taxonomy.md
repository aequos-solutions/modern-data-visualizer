...
> To be able to create a schema extension in Microsoft Graph, you need to "prove" you actually own a tenant to use it. This is done by providing an unique ID of an Azure AD application in your Active Directoy at extension creation time (in the `owner` property). This application doesn't need to do more than just exist in the tenant.
...
- On the **'Modify permissions'** tab, consent the listed permissions:

    !["Teams custom metadata - Graph Explorer permissions"](../../assets/scenarios/teams/graph_explorer_permissions.png){: .center}

- In the **'Request body'** tab, use the following body payloay with your information and press **'Run Query'**:
...
- In the Graph Explorer, using this ID, update the group with the SharePoint taxonomy term values. As a convention, we will use the SharePoint taxonomy term IDs as the main extension properties value and the term labels for the `'AsText'` properties:
